...
## Current handoff
M4.2.1 is accepted and merged through PR #31. M4.2.2 environment preparation is physically accepted on exact clean head `f8275451084af12637e66169ac3c3c06bf78f7cb`; CI #166 passed on the same head. The fixed server environment is under `D:\LocalAI-Prerequisites`: project-local Python 3.12.10, PaddlePaddle GPU 3.2.0, PaddleOCR 3.7.0, NVIDIA driver 596.49, `paddle_device=gpu:0`, and CUDA compilation true. Evidence also confirms `system_python_modified=False`, `ocr_executed=False`, `benchmark_content_created=False`, and `backend_selected=False`.

This accepts environment setup only. PaddleOCR is still an unselected candidate. The next M4.2.2 gate is the controlled benchmark runner/corpus. Keep screenshots, exact ground truth, and raw OCR output local. Review evidence may contain only aggregate metrics keyed by opaque sample/category counts. Explicitly pin the Persian/mixed recognition candidate to `arabic_PP-OCRv5_mobile_rec`; official PP-OCRv5 multilingual documentation lists Persian and English support for that recognition model. Measure strict CER/WER/exact match plus cold/warm latency, failures/timeouts, RAM, GPU VRAM, and footprint. Do not wire OCR into product runtime, add LAN pixel transport, or begin M4.3 before candidate comparison is reviewed.
