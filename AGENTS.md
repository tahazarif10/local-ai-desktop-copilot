...
## Current handoff
M4.1 is accepted and merged. Behavior evidence is anchored to `03f719a1bb4a98a3834348d1f0d53fa50828c92e`; [CI #126](https://github.com/tahazarif10/local-ai-desktop-copilot/actions/runs/35847291181) passed 160/160 Core tests on Ubuntu/Windows, the prior M3.4 runner/provider regressions, controlled raw-provider smoke, and the strict WinUI build. PR #29 merged to `main` as `5ad17ee5276dccefbe5ac4d3e6f7ab845061fdaa`. ADR 0012 is accepted.

The accepted `RegionOfInterestPlanner` remains portable, geometry-only, and bounded. It maps low-resolution `ChangeRegion` coordinates outward into source-frame pixels and maps UIA screen rectangles only through an explicit caller-supplied capture screen projection. It applies bounded padding/association, deterministic ordering, nested-candidate deduplication, count/per-region/total-area budgets, rejects oversized candidates, and never synthesizes a full-frame fallback. It does not crop WGC pixels, run OCR/VLM, inspect prompt text, persist content, log ROI coordinates, or change privacy capabilities/defaults.

M4.2 OCR benchmark and integration is now the next approved implementation gate. Backend selection must be evidence-driven on the fixed client/server hardware and preserve M4.1 ROI bounds, same-epoch `CapturePixels + RunOcr` gating, cancellation, local-only processing, bounded memory, and no implicit whole-frame escalation. Do not begin VLM fallback work before M4.2 acceptance.
