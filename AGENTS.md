...
## Current handoff
The M3.4 runtime-integration behavior is accepted at behavior-bearing head `979ed5a2318d32ff151d2950f7ace77ec274d601` through PR #24. Slice 1 remains the bounded portable admission policy from PR #19 / ADR 0010. Slice 2 is accepted under ADR 0011 from physical behavior head `44d4752864372116a911de2ae3acf611ef033c1e`, session `2a7d17af-4d9b-4ddc-80ac-c727ddd60dc7`, which selected the existing in-process UIA worker after real blocking-provider recovery before the 10-second deadline, bounded shutdown, `joined=True`, `InProcessCandidate`, and sentinel PASS.

The final clean physical M3.4 candidate is `26c3290bf196701473b558da657d5c39c8c97e8a`. On 2026-09-23 the one-command matrix passed: denied session `0cc6c55e-9141-4c32-bfab-3951c808b702`, allowed session `330a2f52-d8f2-4d5c-843d-7149caf9894a`, and provider-regression session `c3cfd465-849e-4e27-984d-f16d4ee6c0b1`. It proved denial before automatic dispatch, Meaningful/Large automatic dispatch and debounce, metadata-only user-question routing, Disarm no-post-dispatch, redaction, runtime-before-worker teardown, joined UIA shutdown, and preservation of the in-process recovery invariant. CI #115 passed the portable/Windows suites, PowerShell parsing, both M3.4 wrapper validations, controlled raw-provider smoke gate, and strict WinUI build on that exact candidate.

Product defaults still deny `ReadUiText`. Automatic semantic UIA requires explicit Armed state, current uncancelled epoch, and `ReadUiStructure | ReadUiText` at admission and publication; returned semantic content is immediately disposed after content-free aggregate observation. Documentation-only descendants after `26c3290` are not new runtime evidence. After PR #24 documentation/review/merge is complete, the next implementation gate is M4.1 region-of-interest planning from merged `main`; do not begin OCR/VLM backend selection before that ROI contract is accepted.
