...
## Current handoff
M4.1 is accepted and merged. Behavior evidence remains anchored to `03f719a1bb4a98a3834348d1f0d53fa50828c92e`; PR #29 merged the ROI planner to `main` as `5ad17ee5276dccefbe5ac4d3e6f7ab845061fdaa`. M4.2 is active on `dev/m4-2-ocr-benchmark`.

M4.2.1 is an evidence-first benchmark/preflight slice under proposed ADR 0013. The branch adds portable strict OCR scoring plus `run-m4-2-ocr-preflight.ps1`. The preflight is non-elevated and content-free: it records target hardware, Windows.Media.Ocr language availability, Tesseract availability/`fas+eng`, Python/PaddleOCR runtime metadata, and NVIDIA metadata when present. It must not capture pixels, execute OCR, install dependencies/models/language packs, or select a backend.

Initial candidate groups are legacy Windows.Media.Ocr, Tesseract 5 Persian+English, and PaddleOCR PP-OCRv5 Persian/English. Microsoft's newer Windows AI Text Recognition API remains outside the current fixed-hardware benchmark because official documentation still requires an NPU. No candidate is an accepted product dependency yet.

M4.2.1 requires CI parse/`-ValidateOnly` PASS plus a physical client preflight. Run the same preflight on the fixed AI server before using a server/GPU candidate in M4.2.2. The controlled benchmark must keep real screenshots, ground truth, and OCR output local and report only aggregate CER/WER/exact-match, latency, memory/VRAM, footprint, failures, and cancellation evidence. Do not integrate OCR, add LAN pixel egress, or begin M4.3 VLM work until benchmark evidence is reviewed and a backend/topology is explicitly accepted.
