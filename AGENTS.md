...
## Current handoff
M4.2.1 benchmark contract/preflight is accepted on `dev/m4-2-ocr-benchmark`. Accepted head `d8f12b00524934138115f22cc8bd7149e20f4452`; CI #137 passed portable/Windows Core tests, Windows PowerShell parsing, the M4.2 `-ValidateOnly` preflight gate, prior M3.4 regressions, and the strict WinUI build. ADR 0013 is accepted; PR #31 review/merge remains before repository state advances.

Physical client preflight: i7-6700K, 31.9 GiB RAM, AMD R9 M395X, legacy Windows OCR only `en-US`, no Tesseract, Python 3.14.7, no Paddle/PaddleOCR. Physical server preflight: i5-12450HX, 15.7 GiB RAM, RTX 3050 6GB Laptop GPU / NVIDIA driver 596.49 / 6144 MiB VRAM, legacy Windows OCR only `en-US`, no Tesseract, Python 3.14.0b2, no Paddle/PaddleOCR. Both runs were non-elevated, clean-tree, content-free, and executed no OCR or installation.

M4.2.2 controlled OCR benchmarking is next after PR #31 merges. Do not modify or replace the existing Python 3.14 installations; use isolated benchmark environments for supported Python versions. Windows.Media.Ocr is currently only an English baseline unless a separate language-pack change is approved. Tesseract and PaddleOCR remain unselected candidates. Keep screenshot fixtures, ground truth, and OCR output local; expose only aggregate scoring/performance evidence. Do not integrate OCR into product runtime or begin M4.3 VLM work before benchmark evidence is reviewed.
