...
M4.2.3 is active on `dev/m4-2-3-ocr-integration`. ADR 0014 selects the measured Paddle configuration as the product integration target on the fixed local AI server while keeping portable Core model-agnostic.

The first active slice, M4.2.3a, is transport-free. Its portable OCR integration gate must require Armed state, the same current uncancelled epoch, `CapturePixels + RunOcr`, a non-empty accepted M4.1 ROI plan, and latest-request revalidation before publication. For `LocalAiServer` topology it must additionally require `SendPixelsToLocalServer`. An empty plan is a safe rejection and must never widen to full-frame OCR.

Do not send product pixels over LAN in M4.2.3a. M4.2.3b must first define and physically validate a narrow authenticated/encrypted bounded OCR transport/runtime path with request/epoch/deadline/cancellation/size limits, no-retention behavior, pre-provisioned Paddle models, content-free diagnostics, and deterministic disposal. The product defaults continue to deny `RunOcr` and `SendPixelsToLocalServer`.
