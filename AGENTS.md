...
M4.2.2 controlled OCR benchmarking is accepted and merged through PR #33 at main commit `d780a0bbeb963b0b491caeddaacc42166dd02fd1`; final CI #213 passed. The strongest measured multilingual configuration was PaddleOCR `PP-OCRv5_server_det + arabic_PP-OCRv5_mobile_rec` with strict CER/WER 0.23004695/0.22388060, warm p50/p95 91.392/117.272 ms, measured GPU VRAM delta 562 MiB, and zero failures/timeouts.

M4.2.3a portable OCR integration gating is accepted through PR #35 at functional head `62a64f60a09924c8007ccc2f4dd1560211ab55ea`; CI #216 passed. ADR 0014 carries the measured server-local PaddleOCR configuration forward while keeping portable Core model-agnostic. The gate requires Armed state, the same current uncancelled epoch, `CapturePixels + RunOcr`, a non-empty accepted M4.1 ROI plan, hard ROI-ceiling revalidation, and latest-request revalidation before publication. `LocalAiServer` topology additionally requires `SendPixelsToLocalServer`. Product defaults continue to deny `RunOcr` and `SendPixelsToLocalServer`.

M4.2.3b is now the active gate on `dev/m4-2-3-ocr-integration`. Before any product ROI pixels cross the LAN, define and physically validate a narrow authenticated/encrypted bounded OCR transport/runtime with request/epoch/deadline/cancellation/size limits, no-retention behavior, pre-provisioned Paddle models, content-free diagnostics, and deterministic disposal. Treat the local AI server as an egress boundary. Do not add an unauthenticated temporary pixel path, do not silently download models during product runtime, and do not begin M4.3 VLM until M4.2.3 physical acceptance passes.
