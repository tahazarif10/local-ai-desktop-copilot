...
## Current handoff
M4.2.1 is accepted and merged through PR #31. M4.2.2 server-local environment preparation is accepted and merged through PR #32 at main commit `1486fb673a623b248e9238c747ac1cd49fa73979`; the exact physical environment head remains `f8275451084af12637e66169ac3c3c06bf78f7cb` with CI #166 PASS. The fixed server environment lives under `D:\LocalAI-Prerequisites`: Python 3.12.10, PaddlePaddle GPU 3.2.0, PaddleOCR 3.7.0, driver 596.49, `gpu:0`, CUDA compilation true.

M4.2.2 controlled benchmarking is active on `dev/m4-2-2-benchmark-runner`. The runner must keep models, corpus, temporary files, results, and ground truth outside Git. It uses opaque sample IDs for seven required UI categories, explicitly pins `PP-OCRv5_mobile_det` and `arabic_PP-OCRv5_mobile_rec`, disables document orientation/unwarping/textline-orientation extras, and emits aggregate metrics only. Raw OCR text, screenshots, and ground truth must never be printed, committed, or copied into review.

CI must cover PowerShell parsing/`-ValidateOnly`, Python syntax, scorer parity tests, and manifest/path-traversal tests. Physical acceptance requires model preparation on D:, a complete local corpus, strict CER/WER/exact match, cold/warm latency, failure/timeout, RAM, GPU VRAM, and footprint evidence. PaddleOCR remains unselected until compared with the required baseline(s). Do not integrate OCR into product runtime, add LAN pixel transport, or begin M4.3 before M4.2 evidence is reviewed.
