...
M4.2.2 controlled OCR benchmarking is accepted and merged through PR #33 at main commit `d780a0bbeb963b0b491caeddaacc42166dd02fd1`; final CI #213 passed. The fixed-server evidence closed the initial candidate search. The strongest multilingual configuration was PaddleOCR `PP-OCRv5_server_det + arabic_PP-OCRv5_mobile_rec` with strict CER/WER 0.23004695/0.22388060, warm p50/p95 91.392/117.272 ms, measured GPU VRAM delta 562 MiB, and zero failures/timeouts. Tesseract `tessdata_fast` and `tessdata_best` were materially less accurate/slower; legacy Windows Media OCR remained English-only. The Paddle cuDNN compile/runtime mismatch remains a recorded compatibility risk.

M4.2.3a is merged through PR #34 at main `56f8aa946981c46f51bf506eba784f1b1c9bf54a`. ADR 0014 selects the measured Paddle configuration as the product integration target on the fixed local AI server while keeping portable Core model-agnostic.

The accepted M4.2.3a portable gate requires Armed state, the same current uncancelled epoch, `CapturePixels + RunOcr`, a non-empty accepted M4.1 ROI plan, and latest-request revalidation before publication. For `LocalAiServer` topology it additionally requires `SendPixelsToLocalServer`. An empty plan is a safe rejection and never widens to full-frame OCR.

M4.2.3b is active on `dev/m4-2-3b-ocr-transport` in draft PR #36. It must physically validate the narrow pinned-TLS/HMAC bounded OCR transport/runtime on the fixed client/server pair before acceptance. Product ROI pixels may cross the LAN only after the same-epoch `CapturePixels + RunOcr + SendPixelsToLocalServer` gate, only for bounded M4.1 ROIs, with deadline/cancellation/latest rejection, pre-provisioned Paddle models, content-free diagnostics, and deterministic client-side content disposal. Product defaults continue to deny `RunOcr` and `SendPixelsToLocalServer`.

Do not begin M4.3 VLM until M4.2.3 product OCR integration passes the fixed client/server acceptance matrix.
