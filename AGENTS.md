...
M4.1 is active on `dev/m4-1-roi-planner`. The behavior-bearing acceptance candidate is `03f719a1bb4a98a3834348d1f0d53fa50828c92e`; CI #126 passed 160/160 Core tests on Ubuntu/Windows, the prior M3.4 runner/provider regressions, and the strict Windows build. ADR 0012 is accepted on PR #29; review/merge remains before `main` advances. The slice is a portable, geometry-only `RegionOfInterestPlanner`. It maps low-resolution `ChangeRegion` coordinates outward into source-frame pixels and maps UIA screen rectangles only through an explicit caller-supplied capture screen projection. Background planning uses only UIA rectangles associated with the observed change envelope and otherwise falls back to the mapped change region; user-question planning may use caller-ordered UIA bounds without a concurrent change.
