...
Accepted M3.2 preserves that ownership/gating path and replaces the M3.1-only manual vtable ABI with pinned CsWin32 source generation. One Element-scope cache request asks for exactly 27 non-text Boolean/numeric properties. Starting at `ElementFromHandleBuildCache`, the worker walks Control View breadth-first with per-node BuildCache sibling/child calls. It records cached `IsContentElement` as the selective Content View marker rather than performing a second traversal, and it never enters Raw View or issues an unbounded descendants query.
...
Accepted M3.3 in PR #18 adds text only behind a separate `ReadUiText` decision that is checked before queueing and again at publication. Ordinary product and diagnostic launches deny it; `run-debug.ps1 -EnableUiText` is the explicit, expiring diagnostic opt-in. The worker selects at most 32 content/on-screen/non-password nodes from the already bounded M3.2 Control View, prioritizes focus then Window control type then breadth-first index, and reads only Name, advertised ValuePattern state/value, and advertised TextPattern visible ranges. It never uses DocumentRange or an action pattern.

The semantic phase independently caps 64 strings, 1,024 UTF-16 code units per string, 32 visible ranges, 16 KiB retained UTF-8, 800 ms, a 24 KiB estimated result, and a five-second lifetime. Content is held in clearable buffers with a finalizer fallback; consumer completion, expiry, stale/latest rejection, and capability loss dispose the complete semantic result. Diagnostics and the view receive only aggregate selection/exclusion counts, budgets, timing, and `content=redacted`. Functional head `3dccdbc24fc60093f46f903dec4f7ca04c08dc14` passed 124 portable tests on Ubuntu and Windows plus the strict build; CI #43 parsed both runners and repeated every automated gate. Physical provider/privacy/budget evidence and clean one-command session `b0af762a-6949-463f-98bf-aa1a0956ea87` passed stale clearing, higher-integrity denial, prior UIA regressions, held-work joined teardown, and the prohibited-content scan. ADR 0009 records the accepted boundary.
...
M4.2 uses an evidence-first benchmark boundary under proposed [ADR 0013](decisions/0013-evidence-gated-ocr-benchmark.md). Candidate discovery and target-machine preflight are separate from OCR execution: preflight records only hardware/runtime/language availability and cannot install dependencies, capture pixels, execute OCR, or select a backend. The initial measured candidate groups are legacy `Windows.Media.Ocr`, Tesseract 5 with Persian+English traineddata, and PaddleOCR PP-OCRv5 Persian/English. These are replaceable benchmark subjects, not architecture commitments.

Microsoft's current Windows AI Text Recognition API remains NPU-only, so it is excluded from the fixed-machine benchmark rather than treated as a failing candidate. Legacy Windows OCR is retained as a zero-additional-runtime baseline only when the target device actually reports suitable recognizer languages. Tesseract provides a CPU-oriented baseline with official Persian and English data. PaddleOCR provides a multilingual Persian/English path suitable for measuring both CPU and the fixed local-server GPU. The project will not infer final topology from library features; client/server placement is chosen only from target-hardware evidence.
