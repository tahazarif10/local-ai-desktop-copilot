...
# default, while allowing a higher-level acceptance harness to re-run the exact
# same measurement logic on its own clean candidate branch. Only the branch
# provenance literal is changed; measurement behavior and evidence gates remain
# unchanged.
...
if ($ValidateOnly) {
    ...
    if (-not $patchedSource.Contains(('if ($branch -ne "{0}")' -f $ExpectedBranch))) {
